...
async def manage_scene(
    ...
    name: Annotated[str | None, "Scene name."] = None,
    path: Annotated[str | None, "Scene path."] = None,
    build_index: Annotated[int | str | None,
                           "Unity build index (quote as string, e.g., '0')."] = None,
    # --- scene_view_frame params ---
    scene_view_target: Annotated[str | int | None,
                                 "GameObject reference for scene_view_frame (name, path, or instance ID)."] = None,
    # --- get_hierarchy paging/safety ---
    parent: Annotated[str | int | None,
                      "Optional parent GameObject reference (name/path/instanceID) to list direct children."] = None,
    page_size: Annotated[int | str | None,
                         "Page size for get_hierarchy paging."] = None,
    cursor: Annotated[int | str | None,
                      "Opaque cursor for paging (offset)."] = None,
    max_nodes: Annotated[int | str | None,
                         "Hard cap on returned nodes per request (safety)."] = None,
    max_depth: Annotated[int | str | None,
                         "Accepted for forward-compatibility; current paging returns a single level."] = None,
    max_children_per_node: Annotated[int | str | None,
                                     "Child paging hint (safety)."] = None,
    include_transform: Annotated[bool | str | None,
                                 "If true, include local transform in node summaries."] = None,
    # --- Multi-scene editing params ---
    scene_name: Annotated[str | None,
                          "Scene name for multi-scene operations."] = None,
    scene_path: Annotated[str | None,
                          "Full scene path (e.g. 'Assets/Scenes/Level2.unity')."] = None,
    target: Annotated[str | int | None,
                      "GameObject reference (name, path, or instanceID) for move_to_scene."] = None,
    remove_scene: Annotated[bool | str | None,
                            "For close_scene: true to fully remove, false to just unload."] = None,
    additive: Annotated[bool | str | None,
                        "For load: true to open scene additively (keeps current scene)."] = None,
    # --- Scene template ---
    template: Annotated[str | None,
                        "For create: scene template ('empty', 'default', '3d_basic', '2d_basic'). Omit for empty scene."] = None,
    # --- Scene validation ---
    auto_repair: Annotated[bool | str | None,
                           "For validate: true to auto-fix missing scripts (undoable)."] = None,
) -> dict[str, Any]:
