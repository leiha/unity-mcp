...
async def manage_prefabs(
    ...
    prefab_path: Annotated[str | None, "Prefab asset path (e.g., Assets/Prefabs/MyPrefab.prefab)."] = None,
    target: Annotated[str | None, "Target GameObject: scene object for create_from_gameobject, or object within prefab for modify_contents (name or path like 'Parent/Child')."] = None,
    allow_overwrite: Annotated[bool | None, "Allow replacing existing prefab."] = None,
    search_inactive: Annotated[bool | None, "Include inactive GameObjects in search."] = None,
    unlink_if_instance: Annotated[bool | None, "Unlink from existing prefab before creating new one."] = None,
    # modify_contents parameters
    position: Annotated[list[float] | dict[str, float] | str | None, "New local position [x, y, z] or {x, y, z} for modify_contents."] = None,
    rotation: Annotated[list[float] | dict[str, float] | str | None, "New local rotation (euler angles) [x, y, z] or {x, y, z} for modify_contents."] = None,
    scale: Annotated[list[float] | dict[str, float] | str | None, "New local scale [x, y, z] or {x, y, z} for modify_contents."] = None,
    name: Annotated[str | None, "New name for the target object in modify_contents."] = None,
    tag: Annotated[str | None, "New tag for the target object in modify_contents."] = None,
    layer: Annotated[str | None, "New layer name for the target object in modify_contents."] = None,
    set_active: Annotated[bool | None, "Set active state of target object in modify_contents."] = None,
    parent: Annotated[str | None, "New parent object name/path within prefab for modify_contents."] = None,
    components_to_add: Annotated[list[str] | None, "Component types to add in modify_contents."] = None,
    components_to_remove: Annotated[list[str] | None, "Component types to remove in modify_contents."] = None,
    create_child: Annotated[dict[str, Any] | list[dict[str, Any]] | None, "Create child GameObject(s) in the prefab. Single object or array of objects, each with: name (required), parent (optional, defaults to target), source_prefab_path (optional: asset path to instantiate as nested prefab, e.g. 'Assets/Prefabs/Bullet.prefab'), primitive_type (optional: Cube, Sphere, Capsule, Cylinder, Plane, Quad), position, rotation, scale, components_to_add, tag, layer, set_active. source_prefab_path and primitive_type are mutually exclusive."] = None,
    delete_child: Annotated[str | list[str] | None, "Child name(s) or path(s) to remove from the prefab. Supports single string or array for batch deletion (e.g. 'Child1' or ['Child1', 'Child1/Grandchild'])."] = None,
    component_properties: Annotated[dict[str, dict[str, Any]] | None, "Set properties on existing components in modify_contents. Keys are component type names, values are dicts of property name to value. Example: {\"Rigidbody\": {\"mass\": 5.0}, \"MyScript\": {\"health\": 100}}. Supports object references via {\"guid\": \"...\"}, {\"path\": \"Assets/...\"}, or {\"instanceID\": 123}. For Sprite sub-assets: {\"guid\": \"...\", \"spriteName\": \"<name>\"}. Single-sprite textures auto-resolve."] = None,
) -> dict[str, Any]:
