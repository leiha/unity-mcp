...
async def read_console(
    ctx: Context,
    action: Annotated[Literal['get', 'clear'] | None,
                      "Get or clear the Unity Editor console. Defaults to 'get' if omitted."] = None,
    types: Annotated[list[Literal['error', 'warning',
                                  'log', 'all']] | str | None,
                     "Message types to get (accepts list or JSON string)"] = None,
    count: Annotated[int | str | None,
                     "Max messages to return in non-paging mode (accepts int or string, e.g., 5 or '5'). Ignored when paging with page_size/cursor."] = None,
    filter_text: Annotated[str | None, "Text filter for messages"] = None,
    page_size: Annotated[int | str | None,
                         "Page size for paginated console reads. Defaults to 50 when omitted."] = None,
    cursor: Annotated[int | str | None,
                      "Opaque cursor for paging (0-based offset). Defaults to 0."] = None,
    format: Annotated[Literal['plain', 'detailed',
                              'json'] | None, "Output format"] = None,
    include_stacktrace: Annotated[bool | str | None,
                                  "Include stack traces in output (accepts true/false or 'true'/'false')"] = None,
) -> dict[str, Any]:
